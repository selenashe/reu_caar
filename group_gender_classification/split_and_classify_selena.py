...
def crop_faces(original_image_name, cropped_image_save_path):
    ...
    results = model(f'examples/{original_image_name}.jpg')  # predict on an image
    ...
    im = Image.open(f'examples/{original_image_name}.jpg')
    im = im.convert('RGB')

    for i in range(box_amount):
        im_new = im.crop((left[i], top[i], right[i], bottom[i]))
        im_new.save(f'{cropped_image_save_path}{original_image_name}_{i}.jpg')
...
for file in os.listdir(cropped_image_save_path):
    if file.startswith(original_image_name):
